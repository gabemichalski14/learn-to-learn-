...
/** The sprout character drawing on its own, so it can be reused for the corner
 *  buddy and for the celebration crowd. */
export function SproutGlyph({ mood = 'idle' }: { mood?: MascotMood }) {
  return (
...
/**
 * Sprouty — the corner buddy for the Playful theme. Bobs while idle, hops when
 * the child is right, gives a sympathetic wobble when not. Decorative only.
 */
export function Mascot({ mood = 'idle', className }: Props) {
  return (
    <div className={`mascot mascot--${mood}${className ? ' ' + className : ''}`} aria-hidden="true">
      <SproutGlyph mood={mood} />
    </div>
  );
}
