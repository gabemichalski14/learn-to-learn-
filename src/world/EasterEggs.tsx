 ...
 */
type EggType = 'peek' | 'clover' | 'butterfly' | 'star';

interface Egg { type: EggType; key: number; left: number; top: number; side: 1 | -1; caught: boolean; }
...
const LIFETIME: Record<EggType, number> = { peek: 4200, clover: 9000, butterfly: 7200, star: 1600 };

export function EasterEggs({ tier }: { tier: number }) {
  ...
  useEffect(() => {
    ...
    const schedule = () => {
      ...
      const id = window.setTimeout(() => {
        if (!alive) return;
        const chance = 0.32 + t * 0.1; // tier 0 ≈ 1-in-3 ticks … tier 5 ≈ 4-in-5
